...
class Encryption extends JPanel implements ActionListener{       // 암호화 클래스
	...
	public void SnailArray(int num) {      // 달팽이 배열 값 초기화
		int x = 8;
		int y = 16;
		char c = 0;
		int i;
    ...
    public void first (String word, String key) {         // 초기단계, 평문 나눔, 키 2개 생성
    ...
    public void XOR() {               // XOR 과정
    	...
    	int num=0;
    	SnailArray(num % 4);
    	for(int m=0; m<key1.length(); m++) {	      // 전치, 배열 회전
    		if(m>=1) {
    			num++;
    			SnailArray(num % 4);
    			Preposition();
    		...
    		int index;
    	for(int i=0; i<code2.length(); i++) {          // 달팽이 배열 치환
    		index = 0;
    		char ask = change2(code2.charAt(i));
    		for(int j=0; j<16; j++)
    			for(int k=0; k<8; k++) {
    				if(ask == index) {
    					char c = snail[j][k];
    	...
    	for(int i=0,j=0; i < code1.length(); i++,j++) {        // 왼쪽 평문 XOR
    		if(j == key2.length())
    			j = 0;
    	...
    	for(int i=0,j=0; i < code2.length(); i++,j++) {        // 오른쪽 평문 XOR
    		if(j == key1.length())
    			j = 0;
        ...
        result.append(code1);
   	    result.append(code2);              // 암호문 두개를 붙여서 최종암호문 생성
